...
def preprocess_data(df):
    ...
    # One-hot encoding to match teammate's approach
...
def create_visualizations(y_true, y_pred, metrics, geo_results_df, output_path):
    print("="*80)
    print("STEP 8: CREATING VISUALIZATIONS (WITH LARGER FONTS)")
    print("="*80)
...
def save_detailed_results(metrics, geo_results_df, labeled_percentage, output_dir):
    ...
    with open(f"{output_dir}/semi_supervised_summary.txt", 'w') as f:
        f.write("ROBUSTNESS CHECK SUMMARY\n")
        f.write(f"Algorithm: XGBoost\n")
        f.write(f"Labeled Data: {labeled_percentage*100}%\n")
...
def run_semi_supervised_analysis(labels_path, values_path, labeled_percentage=0.5, output_dir='./report'):
    ...
    print("Training XGBoost for Robustness Check...")
    ...
    print(f"Done! ARI: {metrics['ARI']:.4f}")
